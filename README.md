...
A global install keeps itself current. The top bar offers a newer version when one is out, and unless settings say otherwise Toyon installs it and restarts on its own while no tab is open and nothing is running. `toyon update` does the same from a terminal. Updates only come through the registry npm is set up for, and `TOYON_UPDATES=off` turns them off for a machine.
