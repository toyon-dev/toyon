...
export async function askFreshAgent(
  ...
  const prepared = await prepareLaunch(cwd, spec);
  ...
  const app = acp
    ...
    .onRequest(acp.methods.client.session.requestPermission, (c) => decideUnattended(c.params, prepared.bounds, cwd))
  ...
  const link = spawnAcp(app, agents.launch(spec, prepared), cwd, `ask:${agentId}`);
  try {
