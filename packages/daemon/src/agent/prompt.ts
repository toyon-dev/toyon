...
export const SYSTEM_APPEND = [
  ...
  'Toyon previews the project in a browser panel by running the commands in toyon.json at the repo root, shaped like {"procs": {"web": "bun run dev --port $PORT"}, "setup": ["bun install"], "check": "bun run check"}: every proc must listen on $PORT, which toyon sets differently for each worktree, setup runs once in every new worktree, and check (optional) runs after each of your turns and must pass before the work can be merged.',
