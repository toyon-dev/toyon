// The OS folder dialog, behind the new-project form's folder button. It opens on the machine the
...
export interface FolderDialog {
  ...
  choose(start: string): Promise<string | null>;
...
export function folderDialog(): FolderDialog {
  ...
  const ask = async (start: string): Promise<string | null> => {
