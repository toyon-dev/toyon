...
export interface SweepDeps {
  ...
  /** its agent is working, asking, or holding a message */
...
export class ArchiveSweep {
  ...
  async sweep(): Promise<void> {
    const afterMs = this.afterMs;
    if (afterMs === null) return;
    const now = (this.d.now ?? Date.now)();
    for (const repo of this.d.state.repos) {
      const rows = this.d.state.worktrees.filter((w) => w.repoId === repo.id && w.kind !== "spare");
      const facts = (counts: ArchiveFacts["counts"]): ArchiveFacts => ({
        now,
        afterMs,
        rows,
      ...
      const beforeGit = facts(() => ({ dirty: 0, ahead: 0 }));
      const maybe = rows.filter((w) => archiveReason(w, beforeGit) !== null);
      if (maybe.length === 0) continue;
      // git now rather than the rail's cached counts, for the rows in question and their siblings
      const fresh = new Map<string, { dirty?: number; ahead?: number }>();
      for (const w of rows) {
      ...
      const measured = facts((id) => fresh.get(id) ?? {});
      for (const w of maybe) {
        const reason = archiveReason(w, measured);
        if (!reason) continue;
